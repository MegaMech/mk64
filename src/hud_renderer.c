...
void func_8004C6FC(s16 x, s16 y, u8 *texture, u32 width, u32 height)
{
  s32 temp_t5;
  s32 temp_v0_3;
  s32 i;
  ...
  s32 tempX = x - (width / 2);
  s32 tempY = y - (height / 2);
    
  //u32 var_t2;
  //y = y - (height / 2);
  temp_t8 = (width * height) * 4; // 131,072
  tex = texture;
  var_s6 = temp_t8;
  //var_t2 = temp_t3;
  gSPDisplayList(gDisplayListHead++, D_0D007EF8);
  ...
  temp_t3 = temp_t8 >> 12; // 32
    
  if (temp_t8 & 0xFFF) // 0
  {
    temp_t3 = temp_t3 + 1;
  }
  var_s0 = height / temp_t3;
 // i = 0;
    end = ((s32) ((width * height)));
  for (i = 0;  i < ((s32) end); i++) {
  // while(i < (s32) temp_t8) {
    temp_t5 = (width * var_s0) * 4;
    func_80043C28(tex, width, (s32) var_s0);
    func_8004B950(tempX, tempY, (s32) width, (s32) var_s0, 1);
    temp_t5 = var_s6 - temp_t5;
    tex += temp_t5;
    if (temp_t5 < 0)
    {
      var_s0 = var_s6 / width;
...
void func_8004C8D4(s16 x, s16 y) {
    func_8004C6FC(x, y, gMarioKart64LogoPtr, 256, 128);
}
...
void func_80050E34(s32 playerId, s32 arg1) {
    ...
    if ((gCurrentCourseId == COURSE_YOSHI_VALLEY) && (lapCount < 3)) {
        gSPDisplayList(gDisplayListHead++, D_0D007DB8);
        gDPLoadTLUT_pal256(gDisplayListHead++, gTLUTPortraitBombKartAndQuestionMark);
        rsp_load_texture(gTexturePortraitQuestionMark, 32, 32);
        object = &gObjectList[objectIndex];
        object->pos[0] = object->unk_028[0] + ((f32) (spD0 + 32));
        ...
        if (player->effects & 0x200) {
            func_8004B614((s32) D_801656C0, (s32) D_801656D0, (s32) D_801656E0, 128, 128, 128, (s32) gObjectList[objectIndex].unk_0A0);
        } else {
            set_transparency((s32) gObjectList[objectIndex].unk_0A0);
        }
        rsp_load_texture(gPortraitTextures[characterId], 32, 32);
        object = &gObjectList[objectIndex];
        object->pos[0] = object->unk_028[0] + ((f32) (spD0 + 32));
        object->pos[1] = object->unk_028[1] + ((f32) (spC4 + spCC));
        object->pos[2] = object->unk_028[2];
        ...
        if (spB8 != 0) {
            func_80042330(spD0 + 38, (spC4 + spCC) + 4, 0, 1.0f);
        } else {
            func_80042330(spD0 + 27, (spC4 + spCC) + 4, 0, 1.0f);
        }
        gSPDisplayList(gDisplayListHead++, D_0D006950);
        if ((player == gPlayerOne) && (gScreenModeSelection == SCREEN_MODE_1P)) {
            gSPDisplayList(gDisplayListHead++, D_0D007A40);
            func_8004B35C(D_8018D3E4, D_8018D3E8, D_8018D3EC, 255);
            func_80044924(gTextureCharacterPortraitBorder, 32, 32);
            rsp_set_matrix_transformation(object->pos, object->unk_0BE, object->sizeScaling);
            gSPDisplayList(gDisplayListHead++, D_0D0069E0);
        }
    }
...
void func_80052590(s32 cameraId) {
    ...
    func_80046F60(gObjectList[temp_s0].activeTLUT, gObjectList[temp_s0].activeTexture, 0x00000020, 0x00000040, 5);
    D_80183E80[0] = gObjectList[temp_s0].unk_0B2[0];
    D_80183E80[2] = gObjectList[temp_s0].unk_0B2[2];
    if ((D_8018CFB0 != 0) || (D_8018CFC8 != 0)) {
        for (var_s2 = 0; var_s2 < 40; var_s2++) {
            temp_s0 = D_8018C3F0[var_s2];
            if (temp_s0 == -1) continue;

            if ((gObjectList[temp_s0].state >= 2) && (gMatrixHudCount < 0x2EF)) {
                D_80183E80[1] = func_800418AC(gObjectList[temp_s0].pos[0], gObjectList[temp_s0].pos[2], temp_s7->pos);
                func_800431B0(gObjectList[temp_s0].pos, D_80183E80, gObjectList[temp_s0].sizeScaling, D_0D0062B0);
            }
        }
    }
    if ((D_8018CFE8 != 0) || (D_8018D000 != 0)) {
        for (var_s2 = 0; var_s2 < 30; var_s2++) {
            temp_s0 = D_8018C630[var_s2];
            if (temp_s0 == -1) continue;
...
void func_80053870(s32 cameraId) {
    ...
    for (var_s2 = 0; var_s2 < D_8018C630_SIZE; var_s2++) {
...
void func_80054BE8(s32 cameraId) {
    ...
    D_80183E80[0] = 0;
    for (var_s0 = 0; var_s0 < D_8018C630_SIZE; var_s0++) {
        temp_a0 = D_8018C630[var_s0];
